...
test('editable flow graph should provide history stack and undo/redo APIs', async () => {
  ...
  assert.ok(src.includes('bulkPropertyFieldsForSelection'))
  assert.ok(src.includes('function resolveCommonPropertyFieldsByNodeIds'))
  assert.ok(src.includes('function patchNodesByIds('))
  assert.ok(src.includes('function patchSelectionRange(patch: FlowVisualNodePatch)'))
...
test('flow center should expose visual undo/redo buttons and keyboard shortcuts', async () => {
  ...
  assert.ok(src.includes('readonlyFlowVisualBulkPanelVisible'))
  assert.ok(src.includes('setReadonlyFlowVisualBulkFieldDraft'))
  ...
  assert.ok(propertySrc.includes('多选后可在扩展区域批量更新'))
