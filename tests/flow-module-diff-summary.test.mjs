...
test('flow center should use module diff summary in save/publish confirm and publish logs', async () => {
  ...
  assert.ok(src.includes("from '/infra/repository/flowModuleRepository'"))
  assert.ok(src.includes('loadFlowModulePublishLogsFromRepository()'))
  assert.ok(src.includes('saveFlowModulePublishLogsToRepository(flowModulePublishLogs.value)'))
  assert.ok(!src.includes('uni.getStorageSync(FLOW_MODULE_PUBLISH_LOG_KEY)'))
  assert.ok(!src.includes('uni.setStorageSync(FLOW_MODULE_PUBLISH_LOG_KEY'))
