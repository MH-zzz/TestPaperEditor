...
test('flow center should use module diff summary in save/publish confirm and publish logs', async () => {
  ...
  assert.ok(lifecycleSrc.includes("from '/domain/flow-module/usecases/buildModuleDiffSummary'"))
  assert.ok(lifecycleSrc.includes('buildModuleDiffSummary({'))
  assert.ok(lifecycleSrc.includes('formatModuleDiffSummary(diffSummary)'))
  assert.ok(lifecycleSrc.includes("from '/infra/repository/flowModuleRepository'"))
  assert.ok(lifecycleSrc.includes('loadFlowModulePublishLogsFromRepository()'))
  assert.ok(lifecycleSrc.includes('saveFlowModulePublishLogsToRepository(flowModulePublishLogs.value)'))
  assert.ok(!lifecycleSrc.includes('uni.getStorageSync(FLOW_MODULE_PUBLISH_LOG_KEY)'))
  assert.ok(!lifecycleSrc.includes('uni.setStorageSync(FLOW_MODULE_PUBLISH_LOG_KEY'))
  assert.ok(lifecycleSrc.includes('appendFlowModulePublishLog('))
  assert.ok(src.includes('showPublishLogs'))
  assert.ok(lifecycleSrc.includes('发布日志'))
})
