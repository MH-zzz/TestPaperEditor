...
让非技术人员（编辑、产品）能够通过**拖拽组件**和**连线**，直观地设计出复杂的考试交互流程，并自动生成符合引擎规范的 `FlowModule` 数据。
...
3.  **正向编译器 (Forward Compiler)**
    ...
    *   [x] 保存/发布闸门已接入可视流程状态：未应用变更或不可编译都会阻断，并可定位到可视节点。

4.  **交互增强**
    *   [x] 限制连线规则：当前画布不暴露自由连线编辑，连线由线性节点顺序自动生成（天然单向）。
    ...
    *   [x] 已支持撤销/重做（按钮 + Ctrl/Cmd+Z, Ctrl/Cmd+Shift+Z, Ctrl/Cmd+Y）。
    *   [x] 已支持快捷导航与复制（↑/↓ 切换节点，Ctrl/Cmd+D 复制选中节点）。
    *   [x] 已支持线性约束面板（单入口/单出口/无分支/无环路/全连通）与错误点击定位到节点。
